...
class MetaAgent():
    ...
    def UCB():
        ...
        # ucb_inst_score = np.zeros((T,))       # reward for round t
        # ucb_score = np.zeros((T,))            # cumulative reward for round t
        ...
        #TODO can we calculate regret ?
        #ucb_regret =  np.zeros((T,))        # regret for round t

        # iterate over the "time rounds" of the dataset
        #for round_no in range(T):

        # pick the arm/server with the biggest UCB
        picked_arm = np.argmax(ucb)
        # "play" that arm and get a score
        ucb_inst_score[round_no] = float(dataset[picked_arm][round_no])

        #if ucb_inst_score[round_no] == min_delay: ucb_perfect += 1

        # update the estimate and the UCB for the played arm
        ############
        
        for agent_id in average_performance.index:
            avg_reward = average_performance[agent_id]
            play_count = play_counts[agent_id]
            ucb[agent_id] = avg_reward + np.sqrt(2 * np.log(total_plays) / agent_play_count)
            # new update ucb
