...
aimed to give the UCB a headstart by using Machine Learning.
...
Our dataset was created by collecting this data with a custom python script and is placed in the training_data folder.
We used the trainer.py to pre-train the Neural Network model that gives strategy preformance prediction with one simple forward pass, when the negotiator agent 
sees the domain for the first time. 
Then the UCB algorithm takes over and plays the strategy with the highest confidence bound, adapting the confidence bounds accordingly. 